...
public class ChromeManager {
    ...
    private static Properties loadConfig() throws IOException {
        ...
        try {
            // Usa ClassPathResource per caricare dalle resources
            ClassPathResource resource = new ClassPathResource("converter.properties");

            if (resource.exists()) {
                logger.debug("Trovato converter.properties nelle resources");
                try (InputStream is = resource.getInputStream()) {
                    config.load(is);
                    logger.info("Configurazione caricata da converter.properties: {} proprietà", config.size());
                }
            } else {
                throw new FileNotFoundException("convertr.properties non trovato");
            }

        } catch (IOException e) {
            logger.error("Errore nel caricamento di converter.properties dalle resources", e);
        }
    ...
    private static String getBundledChromePath(String os, Properties config) {
        ...
        String chromePath = config.getProperty(configKey);
        if (chromePath != null) {
            // ✅ FIX: Controlla se è path assoluto o relativo
            File chromeFile;
            if (chromePath.startsWith("/") || chromePath.matches("^[A-Za-z]:.*")) {
                // Path assoluto (Linux/Mac inizia con /, Windows con C:)
                chromeFile = new File(chromePath);
            } else {
                // Path relativo dalla directory dell'applicazione
                File baseDir = new File(System.getProperty("user.dir"));
                chromeFile = new File(baseDir, chromePath);
            }
